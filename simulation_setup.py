...
from gpu_acc_se import compute_se_cf_ul
...
def CF_mMIMO_Env(L, K, tau_p, max_power, UEs_power, APs_positions, UEs_positions, square_length, decorr, sigma_sf,
    ...
    wrapped_AP_positions = torch.tile(APs_positions.flatten(), (len(wrap_locations), 1)).T + torch.tile(wrap_locations,
                                                                                                        (L, 1))
    wrapped_UE_positions = torch.tile(UEs_positions.flatten(), (len(wrap_locations), 1)).T + torch.tile(wrap_locations,
                                                                                                        (K, 1))
    ...
    return B_k, signal_CF, interference_CF, pilot_index_CF, APs_positions, UEs_positions
