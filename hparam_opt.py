...
from _utils import TrialEvalCallback, generate_ap_locations, generate_ue_locations, check_and_launch_dashboard
...
def hyperparameters_optimization(study_name, sampler_method, pruner_method, max_total_trials, algo,
                                 ...
                                 n_eval_envs, deterministic_eval, optimization_log_path, verbose, n_eval_episodes,
                                 save_path, storage_url):
    seed = 0
            ...
            if completed_trials < max_total_trials:
                study.optimize(
                    lambda _trial: objective(_trial, algo, env_id, n_envs, env_kwargs, eval_env_kwargs, n_eval_episodes,
                                             ...
                                             optimization_log_path, verbose, seed=seed, save_path=save_path,
                                             no_log=True), n_jobs=1,
...
if __name__ == "__main__":
    ...
    # Log Directory
    log_directory = Path(f"logs_{config['algo']}_{config['reward_method']}")
    log_directory.mkdir(parents=True, exist_ok=True)

    # Hyperparameter Tuning Configuration
    _study_name = f"{config['env_name']}-{config['algo']}-{config['reward_method']}"
    ...
    _optimization_log_path = f"optuna_{config['algo']}_{config['reward_method']}"  # Directory for optimization logs
    _verbose = 3  # Verbosity level
    _save_path = str(log_directory)  # Directory for saving logs
    _storage_url = f"sqlite:///{config['env_name']}.sqlite3"
