...
for n in range(num_of_setups):
    ...
    cf_SE = compute_se(info['signal'], info['interference'], info['predicted_power'], prelog_factor)
print('Total inference: ', time.time() - over_all_time)

cf_SE = compute_se(cf_signal, cf_interference, cf_pred_power, prelog_factor)
